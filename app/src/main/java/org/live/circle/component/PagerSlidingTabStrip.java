...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	private float dividerWidth = 0.33f;

	private int tabTextSize = 14;
	private ColorStateList tabTextColor = ColorStateList.valueOf(0xff333333);
	private Typeface tabTypeface = null;
	private int tabTypefaceStyle = Typeface.NORMAL;

	private int lastScrollX = 0;

	private int tabBackgroundResId = R.drawable.common_ripple_dark_selector;
	...
	public void notifyDataSetChanged() {
		...
		if (pagerAdapter == null) {
			throw new IllegalArgumentException("can not initPagerSlidingTabStrip before set up pagerAdapter");
		}
		tabsContainer.removeAllViews();

		tabCount = pagerAdapter.getCount();
		tabView = new View[tabCount];
		for (int i = 0; i < tabCount; i++) {
			if (pagerAdapter instanceof IconTabProvider) {
				tabView[i] = addIconTab(i, ((IconTabProvider) pagerAdapter).getPageIcon(i));
			} else if (pagerAdapter instanceof CustomTabProvider) {
				tabView[i] = addCustomTab(i, ((CustomTabProvider) pagerAdapter).getTabView(i));
			} else {
				tabView[i] = addTextTab(i, pagerAdapter.getPageTitle(i).toString());
			}
		}
	...
	private View addCustomTab(final int position, View customTab) {
		addTab(position, customTab);
		return customTab;
	}

	private ImageButton addIconTab(final int position, Drawable resIcon) {
		ImageButton tab = new ImageButton(getContext());
		tab.setImageDrawable(resIcon);
		addTab(position, tab);
		return tab;
	}
	...
	public interface OnTabSelectedListener {
		void onTabReselected(View tabView, int position);
	...
	public void setTabText(int position, CharSequence title) {
		if (tabView != null && tabView.length > position && tabView[position] instanceof TextView) {
			((TextView)tabView[position]).setText(title);
		}
	}

	public CharSequence getTabText(int position) {
		if (tabView != null && tabView.length > position && tabView[position] instanceof TextView) {
			return ((TextView)tabView[position]).getText();
		} else {
			return "";
		}
	}
